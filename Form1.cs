...
namespace least_squares_method
{
    public partial class Form1 : Form
    {
        ...
        private void TableAddPoint(object sender, DataGridViewCellEventArgs e)
        {
            ...
            List<List<double>> SpecialCase = new List<List<double>>();
        ...
        List<List<double>> GetUniquePoints(List<List<double>> Points)
        {
            List<List<double>> UniquePoints = new List<List<double>>();

            foreach (List<double> Point in Points)
            {
                if (!UniquePoints.Any(x=> x[0] == Point[0] && x[1] == Point[1]))
                {
        ...
        private void CalculateButton(object sender, EventArgs e)
        {
           ...
           // List<double> Coefficients = SpecialCase()
            List<List<double>> Points = GetAllPoints();
            ...
            try
            {
                ...
                if (!Double.IsNaN(Coefficients[0]) && !Double.IsNaN(Coefficients[1]) && Points.Count > 1)
                {
